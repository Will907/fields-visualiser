...
class PointCharge(Point):
    ...
    def get_closest(cls,x,y):
        #Returns the closest charge to the given point
    ...
    @classmethod
    def update_streamstarts(cls):
        #assign each charge a fraction of the number of total streamlines (basestreams)
        basestreams = 20
        qs = tuple(charge.q for charge in cls.charges)
        qsum = np.sum(qs)
        for charge in cls.charges:
            charge.streamstarts = (charge.q*basestreams)//qsum
...
#Aliasing pointcharge list for easy access        
...
def e_field(charges):
    if len(charges) > 0:
        PointCharge.update_streamstarts()
    ...
    for charge in charges:
        if insufficient:
            insufficient = False
            continue
        for i in range(charge.streamstarts):
            if charge.streamstarts < 1:
                insufficient = True
                break
            theta = 2*i*np.pi / charge.streamstarts
